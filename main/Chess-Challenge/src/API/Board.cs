namespace ChessChallenge.API
{
    ...
    public sealed class Board
    {
        ...
        /// This is used for PGN loading. If successful, the move is made and true is returned.
        /// If unsuccessful (e.g., illegal move or invalid SAN), false is returned and the board remains unchanged.
        /// </summary>
        public bool TryMakeMoveFromSan(string sanMoveString, out Move move)
        {
            if (board.TryMakeMoveFromSan(sanMoveString, out Chess.Move coreMove))
            {
                OnPositionChanged();

                // Get piece types for API move
                int movePieceType = PieceHelper.PieceType(board.Square[coreMove.TargetSquareIndex]);
                int capturePieceType = 0;

                if (coreMove.IsEnPassant)
                {
                    capturePieceType = PieceHelper.Pawn;
                }
                else if (board.currentGameState.capturedPieceType != 0)
                {
                    capturePieceType = board.currentGameState.capturedPieceType;
                }

                move = new Move(coreMove, movePieceType, capturePieceType);
                return true;
            }
